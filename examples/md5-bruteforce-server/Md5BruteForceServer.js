...
Md5BruteForceServer.prototype.createTasks = function (request, callback) {
    ...
    this.db.all('SELECT id FROM tasks WHERE expires > ' + now + ' AND done = 0', function(err, rows) {
        if (err !== null) {
            callback(err);
            return;
        }
        ...
        if (!rows.length) {
            ...
            // Create
            self.db.run('INSERT INTO tasks VALUES (NULL, ' + expires + ', 0)', function (err) {
                // get last insert id
                self.db.all("SELECT last_insert_rowid() AS id", function (err, rows) {
                    ...
                    callback(null, [{
                        id: row.id,
                        data: {
                            min: row.id * TASK_STEP,
                            max: (row.id * TASK_STEP) + TASK_STEP,
        ...
        self.db.run('UPDATE tasks SET expires = ' + expires + ' WHERE id = ' + row.id, function (err) {
            if (err) {
                callback(err);
                return;
            }
...
Md5BruteForceServer.prototype.saveTasks = function (request, callback) {
    ...
    var stmt = this.db.prepare("UPDATE tasks SET done = 1 WHERE id = ?");
