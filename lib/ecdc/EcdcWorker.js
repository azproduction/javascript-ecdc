/**
 * @fileOverview JavaScript ECDC Client's Worker Class
 *
 * @author  azproduction
 * @licence Dual licensed under the MIT or GPL Version 2 licenses
 * @version 0.1a
 */
var EcdcWorker = (function(global) {
    /**
     * @constructor
     * @namespace
     *
     * @param {Boolean} [isSynchronous=true]
     */
    var EcdcWorker = function (isSynchronous) {
        var self = this;

        /**
         * Synchronous or not
         *
         * Default true
         *
         * @type Boolean
         */
        this.isSynchronous = true || isSynchronous == null;
        /**
         * Not active by default
         *
         * @type Boolean
         */
        this.active = false;
        /**
         * Regexp to match own localStorage keys
         * 
         * @type RegExp
         */
        this.taskRegExp = new RegExp('^task-');
        /**
         * Is looking for local tasks (in localStorage)
         *
         * @type Boolean
         */
        this.isCheckLocalTasks = true;
        /**
         * Retry timeout
         *
         * @type Number
         */
        this.taskTimeut = this.GET_TASK_TIMEOUT;
        /**
         * Complete but unsent tasks buffer
         *
         * @type Object[]
         */
        this.tasksBuffer = [];

        // Listener for master's messages
        global.addEventListener('message', function (event) {
            self.getMessage(event.data);
        }, false);
    };

    /**
     * Max tasks in buffer
     *
     * @const
     * @type Number
     */
    EcdcWorker.prototype.MAX_TASKS_BUFFER = 1;
    /**
     * Default timeout
     *
     * @const
     * @type Number
     */
    EcdcWorker.prototype.GET_TASK_TIMEOUT = 2e1;        // 2 sec
    /**
     * Default max timeout
     *
     * @const
     * @type Number
     */
    EcdcWorker.prototype.MAX_GET_TASK_TIMEOUT = 12e4;   // 2 minutes
    /**
     * Default computing time, after that time task marks as "overcomputed"
     * and other worker can gat that task
     *
     * @const
     * @type Number
     */
    EcdcWorker.prototype.MAX_TASK_COMPUTING_TIME = 6e4; // 1 minute
    /**
     * REST url
     *
     * @const
     * @type String
     */
    EcdcWorker.prototype.URL = 'http://localhost/task';

    /**
     * Posts message to master
     * 
     * @private
     * @param {String} message
     */
    EcdcWorker.prototype.postMessage = function (message) {
        global.postMessage(message);
    };

    /**
     * Message listener
     *
     * @param {String} message
     */
    EcdcWorker.prototype.getMessage = function (message) {
        switch (message) {
            case 'start':
                self.start();
                break;
            case 'pause':
                self.pause();
                break;
        }
    };

    /**
     * Starts worker
     */
    EcdcWorker.prototype.start = function () {
        this.postMessage('started');
        this.active = true;

        // Clean expired tasks
        this.cleanupTasks();

        // Startup job
        this.checkUnsentTasks(function (err, tasks) {
            if (err === null) {
                if (this.isSynchronous) {
                    this.doTasksSync(tasks);
                } else {
                    this.doTasks(tasks);
                }
            } else {
                // If some error sleep
                this.postMessage('Error (%s) on check unsent tasks', err);
                this.pause();
            }
        }, this);
    };

    /**
     * Pauses worker, worker allowed to finish current task
     */
    EcdcWorker.prototype.pause = function () {
        this.postMessage('paused');
        this.active = false;
    };

    /**
     * Performs Async tasks
     *
     * Task format
     * {String} task.id            from server | any task id
     * {String} task.expires       from server | date string eg.: Sat Apr 30 2011 23:41:19 GMT+0600
     * {Object} task.data          from server | any task data
     * 
     * {String} task.state         from client | task status {progress|complete}
     * {String} task.time          from client | after that time task will be "overcalculated", format like task.expires
     * {Object} task.result        from client | task result
     * {String} task.result.id     from client | task result id must be equal task id
     * {Object} [task.result.data] from client | task result data may be another name
     *
     * @param {Object|Object[]} tasks
     */
    EcdcWorker.prototype.doTasks = function (tasks) {
        var tasksCount = tasks.length,
            self = this,
            results = [];

        function postAllResults(results) {
            // Post all items
            self.postTasks(results, function (err, tasks) {
                if (err === null) {
                    // Do again...
                    this.doTasks(tasks);
                } else {
                    // If some error - sleep
                    this.postMessage('Error (%s) on post tasks', err);
                    this.pause();
                }
            }, self);
        }

        if (!this.active) {
            return;
        }
        
        tasks = [].concat(tasks);

        tasks.forEach(function (task) {
            if (!self.active) {
                return;
            }
            var key = self.createTaskKey(task);
            task.state = 'progress';
            // touch item
            task.time = new Date(+new Date() + self.MAX_TASK_COMPUTING_TIME).toString();
            // push item
            EcdcWorker.Storage.setItem(key, task);
            // Now task marked as in progress
            // Calculate
            self.calculate(task.id, task.data, function (result) {
                // Validate
                task.result = result instanceof Object ? result : {data: {}};
                // Force add id
                task.result.id = task.id;
                // Complete task
                task.state = 'complete';
                // push item
                EcdcWorker.Storage.setItem(key, task);
                results.push(task.result);

                tasksCount--;
                // All done
                if (tasksCount <= 0) {
                    postAllResults(results);
                }
            });
        });
    };

    /**
     * Performs Sync tasks
     *
     * @param {Object|Object[]} tasks
     */
    EcdcWorker.prototype.doTasksSync = function (tasks) {
        if (!this.active) {
            return;
        }
        tasks = [].concat(tasks);
        // Iterate...
        for (var i = 0, c = tasks.length, task, key, results = []; i < c; i++) {
            if (!this.active) {
                return;
            }
            task = tasks[i];
            key = this.createTaskKey(task);
            task.state = 'progress';
            // touch item
            task.time = new Date(+new Date() + this.MAX_TASK_COMPUTING_TIME).toString();
            // push item
            EcdcWorker.Storage.setItem(key, task);
            // Now task marked as in progress
            // Calculate
            task.result = this.calculateSync(task.id, task.data);
            // Validate
            task.result = task.result instanceof Object ? task.result : {data: {}};
            // Force add id
            task.result.id = task.id;
            // Complete task
            task.state = 'complete';
            // push item
            EcdcWorker.Storage.setItem(key, task);
            results.push(task.result);
        }
        // Post all items
        this.postTasks(results, function (err, tasks) {
            if (err === null) {
                // Do again...
                this.doTasks(tasks);
            } else {
                // If some error - sleep
                this.postMessage('Error (%s) on post tasks', err);
                this.pause();
            }
        }, this);
    };

    /**
     * Creates task key
     *
     * @param {Object} task
     *
     * @returns {String}
     */
    EcdcWorker.prototype.createTaskKey = function (task) {
        return 'task-' + task.id;
    };

    /**
     * Get tasks or task
     * 
     * @param {Function} callback callback(err, items)
     * @param thisObject
     */
    EcdcWorker.prototype.getTasks = function (callback, thisObject) {
        var self = this;
        
        thisObject = thisObject || null;

        // Check long tasks
        if (this.isCheckLocalTasks) {
            // Iterate...
            for (var i = 0, c = EcdcWorker.Storage.length, key, item; i < c; i++) {
                key = EcdcWorker.Storage.key(i);
                if (this.isMyTaskData()) {
                    item = global.JSON.parse(EcdcWorker.Storage.getItem(key));
                    // Task is in progress and not expires but it calculates too long...
                    if (item.state === 'progress' && new Date(item.time) <= new Date() && new Date(item.expires) > new Date()) {
                        // touch item
                        item.time = new Date(+new Date() + this.MAX_TASK_COMPUTING_TIME).toString();
                        // push item
                        EcdcWorker.Storage.setItem(key, item);
                        // Task found
                        callback.call(thisObject, null, item);
                        this.postMessage('Get task from cache');
                        return;
                    }
                }
            }
            // No more local tasks
            this.isCheckLocalTasks = false;
        }
        
        // No local tasks found
        // Perform XHR get
        var options = {
            method: 'get',
            url: this.URL,
            callback: function (err, data) {
                if (err !== null) {
                    // Some error, retry
                    self.retry(options);
                    // Log
                    self.postMessage('Error (%s) on get task, retry...', err);
                } else {
                    // Ok
                    self.resetTimeout();
                    callback.call(thisObject, null, data);
                    self.postMessage('Get new task');
                }
            }
        };
        Request(options);
    };

    /**
     * Retryes to perform request
     *
     * @param {Object} options
     */
    EcdcWorker.prototype.retry = function (options) {
        this.taskTimeut *= 2;
        if (this.taskTimeut > this.MAX_GET_TASK_TIMEOUT) {
            this.taskTimeut = this.MAX_GET_TASK_TIMEOUT;
        }
        global.setTimeout(function () {
            Request(options);
        }, this.taskTimeut);
    };

    /**
     * Resets task timeout
     */
    EcdcWorker.prototype.resetTimeout = function () {
        this.taskTimeut = this.GET_TASK_TIMEOUT;
    };

    /**
     * Checks if object under that key is task
     * 
     * @param {String} key
     * 
     * @returns {Boolean} tru if its task
     */
    EcdcWorker.prototype.isMyTaskData = function (key) {
        return this.taskRegExp.test(key);
    };

    /**
     * Cleanups expired or defined tasks
     *
     * @param {String[]} [tasksIds]
     */
    EcdcWorker.prototype.cleanupTasks = function (tasksIds) {
        var i, c, key, item
        // Cleanup listed tasks
        if (tasksIds) {
            for (i = 0, c = tasksIds.length; i < c; i++) {
                EcdcWorker.Storage.removeItem(tasksIds[i]);
            }
            return;
        }
        // Cleanup expired tasks
        for (i = 0, c = EcdcWorker.Storage.length; i < c; i++) {
            key = EcdcWorker.Storage.key(i);
            if (this.isMyTaskData()) {
                item = global.JSON.parse(EcdcWorker.Storage.getItem(key));
                // Task is expired, cleanup
                if (new Date(item.expires) <= new Date()) {
                    EcdcWorker.Storage.removeItem(key);
                }
            }
        }
    };

    /**
     * Checks complete but unsent tasks
     * 
     * @param callback
     * @param thisObject
     */
    EcdcWorker.prototype.checkUnsentTasks = function (callback, thisObject) {
        // Iterate...
        for (var i = 0, c = EcdcWorker.Storage.length, key, item, tasks = {}, empty = true; i < c; i++) {
            key = EcdcWorker.Storage.key(i);
            if (this.isMyTaskData()) {
                item = global.JSON.parse(EcdcWorker.Storage.getItem(key));
                // Task is complete and not expired
                if (item.state === 'complete' && new Date(item.expires) > new Date()) {
                    // Prepare task list...
                    tasks[item.id] = item.data;
                    empty = false;
                }
            }
        }
        
        if (empty) {
            // Get new task or one from cache
            this.getTasks(callback, thisObject);
        } else {
            // Post task
            this.postTasks(tasks, callback, thisObject);
        }
    };

    /**
     * Post tasks to server or collecting them in buffer
     * 
     * @param {Object[]} taskResults
     * @param {Function} callback
     * @param {Object}   thisObject
     */
    EcdcWorker.prototype.postTasks = function (taskResults, callback, thisObject) {
        var self = this,
            postVars;

        this.tasksBuffer = this.tasksBuffer.concat(taskResults);
        if (this.tasksBuffer.length < this.MAX_TASKS_BUFFER) {
            // Continue collecting tasks
            this.getTasks(callback, thisObject);
            return;
        }

        // Buffer full? send data to server
        postVars = global.JSON.stringify(this.tasksBuffer);
        var tasksIds = [];

        for (var i = 0, c = this.tasksBuffer.length; i < c; i++) {
            tasksIds.push(this.createTaskKey(this.tasksBuffer[i]));
        }

        // Clear buffer
        this.tasksBuffer = [];
        // Perform XHR Post
        var options = {
            method: 'post',
            postVars: postVars,
            url: this.URL,
            callback: function (err, data) {
                if (err !== null) {
                    self.retry(options);
                    self.postMessage('Error (%s) on post task, retry...', err);
                } else {
                    self.resetTimeout();
                    // Get tasks
                    callback.call(thisObject, null, data);
                    // Cleanup storage
                    self.cleanupTasks(tasksIds);
                    self.postMessage('Get new task after post');
                }
            }
        };
        Request(options);
        self.postMessage('Post result');
    };

    /**
     * This method performs all calculation -  it synchronous
     *
     * @param {String|Number} id   task id
     * @param {Object}        data task data
     *
     * @returns {Object}
     */
    EcdcWorker.prototype.calculateSync = function (id, data) {
        // Do nothing
        return {id: id, data: data};
    };

    /**
     * This method performs all calculation - it asynchronous
     *
     * @param {String|Number} id   task id
     * @param {Object}        data task data
     * @param {Function}      callback 
     */
    EcdcWorker.prototype.calculate = function (id, data, callback) {
        // Do nothing
        global.setTimeout(function () {
            callback({id: id, data: data});
        }, 9);
    };

    /**
     * @type {Storage}
     */
    EcdcWorker.Storage = global.localStorage;

    /**
     * Performs XHR
     * 
     * @param {Object}   params
     * @param {String}   params.url
     * @param {Function} params.callback       callback(errorCode, json)
     * @param {String}   [params.method='get']
     * @param {Number}   [params.timeout=5000]
     *
     * @throws {Error} Callback, Url?
     */
    var Request = function (params) {
        params = params || {};
        if (!params.callback || !params.url) {
            throw new Error('Callback, Url?');
        }
        params.method = params.method || 'get';
        params.timeout = params.timeout || 5000;

        var xhr = new global.XMLHttpRequest();
        xhr.onreadystatechange = function() {
            try {
                if (xhr.readyState !== 4) {
                    return;
                }
                if (xhr.status === 200) {
                    global.clearTimeout(timerId);
                    params.callback(null, params.thisObject, xhr.contentType.match(/^application\/json/) ? JSON.parse(xhr.responseText) : xhr.responseText);
                } else {
                    global.clearTimeout(timerId);
                    params.callback(xhr.status);
                }

            } catch (e) {
                global.clearTimeout(timerId);
                params.callback(xhr.status);
            }
        };

        xhr.open(params.method, params.url, true);
        xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
        xhr.send(params.method.toLowerCase() === 'post' ? params.postVars : null);

        var timerId = global.setTimeout(function(){
            xhr.abort();
            params.callback(0);
        }, params.timeout);
    };

    return EcdcWorker;
}(this));